...
Under normality, as before given 80% power, assuming a true effect, two studies will report consistently positive results in only `r round(powres[1,1]^2, digits = 3)`% of the time, dropping to `r round(powres[1,2]^2, digits = 3)`% for 10% trimmed means, and `r round(powres[1,3]^2, digits = 3)`% for 20% trimmed means. 

When sampling from a contaminated normal population, now power is only `r round(powres[2,1], digits = 3)`% for the mean, but remains high at `r round(powres[2,2], digits = 3)`% for the 10% trimmed mean and `r round(powres[2,3], digits = 3)`% for the 20% trimmed mean. As a result, positive decision consistency drops to `r round(powres[2,1]^2, digits = 3)`% for the mean, `r round(powres[2,2]^2, digits = 3)`% for 10% trimmed means, and `r round(powres[2,3]^2, digits = 3)`% for 20% trimmed means.
