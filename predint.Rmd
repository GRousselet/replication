...
Prediction interval works as intended. Coverage using CI rule is too conservative. CI overlap is far too liberal. PI for 20% trimmed means has coverage a bit liberal.  
...
Now let's consider a situation in which we perform one experiment with sample size n: if the result is statistically significant, and the effect size is larger than the minimum effect size of interest (0.1), then we carry out a replication with sample size n*2. 
Population is slightly skewed and likely to generate outliers: $g=0.3$ and $h=0.05$. 
The population has mean 0.3 -- so Cohen's d of 0.3 assuming a normal population, but less in practice. 
What proportions of experiments lead to consistent results, meaning that the original and the replication both meet the criteria?  
